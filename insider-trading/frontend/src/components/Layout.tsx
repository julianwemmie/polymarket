...
export default function Layout({ children }: LayoutProps) {
  ...
  const navLinks = [
    { to: "/entities", label: "Investigations" },
      ...
      <nav className="bg-gray-900 border-b border-gray-800 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <Link to="/entities" className="flex items-center gap-3 group">
            ...
            <div className="flex items-center gap-1">
              {navLinks.map((link) => {
                const isActive =
                  link.to === "/entities"
                    ? location.pathname === "/" || location.pathname.startsWith("/entities")
      ...
      <footer className="border-t border-gray-800 py-4">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <p className="text-center text-gray-600 text-xs">
            On-chain analysis tool for educational purposes. Not financial advice. Data sourced from public blockchain
            records and Polymarket APIs.
          </p>
        </div>
      </footer>
    </div>
